...
### 2026-09-22 — dm4800/derive-pipeline — PR #6's builds: all green (closes the entry above)
- **Starting:** closes the Next of the entry above; no new work.
- **Did:** nothing.
- **Checked:** all thirteen checks on PR #6 passed by 04:45 UTC. Each of the four `ubuntu-22.04` job logs shows `bundles no wayland libraries`, then `ran /tmp/opener-check-*/system-bin/xdg-open with the desktop's own environment` and `OPENER CHECK GREEN (1 checked)`. That is the first run of the runtime check on GitHub's runner, and against the CI bundle names, which contain spaces.
- **Found:** nothing wrong.
- **Next / needs:** the mod author merges PR #6.

### 2026-09-22 — dm4800/derive-pipeline — A derive pipeline for proposing fork work upstream
- **Starting:** owner: "merge that into main. i think we need a derive pipeline for opening prs against upstream like we do in emulator-release-repo, just for this repo. its a low-stakes repo in comparison to the emulators." Local `main` fast-forwarded to `bcebceb` (upstream/main plus the three handoff commits; it was at `214cff5`, 22 commits behind). Why a pipeline is now needed: `main` carries fork-only files (`worker_log.md`, `handoff.md`, `AGENTS.md`, `.gitattributes`), so a PR can no longer be cut from it by hand without risking those crossing. About to build `tools/upstream/` on this branch: a derive that produces exactly what a PR publishes, as the diff between upstream's tree and the fork head's tree with fork-only paths filtered out, bounded by the paths the caller names; refusals for a stray path, a head that does not contain `upstream/main`, an empty derive, a reused branch name, and a tripwire hit in the added lines, the commit message or the PR body; the repo's own `validate` checks run on the derived tree; the push guard's decision run offline; a small ledger of prepared and merged PRs. The publish itself stays the owner's, through the fleet hand-off in branch mode, which is proven by PR #6. Written after an initial read-only survey of the release repo's tools, not before it. TASK-CLASS: SIGNIFICANT (T=4 S1 N1 R0 E1 B1 M:new mechanism). Instruments: a self-test with seeded failures, plus a real-data equivalence check (deriving PR #6's change from a fork topic must reproduce `7548a80`'s diff byte for byte). Apprentice waived: the owner rates this repo low-stakes, and the equivalence check is a different artifact class from the self-test.
- **Tool search (H-16):** `toolindex.py --find "derive pull request fork upstream patch filter"` and `"prepare pr against upstream from fork exclude fork-only files"` returned `pcsx2-VR/tools/e2e/suites/70-fork-integrity.sh`, `xbox360-static-recomp/scripts/fork/run-gates.sh` and unrelated validators; none derives a patch. `emulator-release-repo/tools/pr-prepare.py` is the pattern, not a fit: it is bound to that repo's `targets/`, ledgers and watermark, and calls a `tools/release/derive.py` inside the working repo, which this repo does not have. The owner asked for a lightweight version living here.
- **Did:** `tools/upstream/derive-pr.py` (`prepare`, `status`, `record-merged`, `--self-test`) and `tools/upstream/FORK-ONLY` (this guide, the log, the handoff, `.gitattributes`, `tools/upstream`). `AGENTS.md` gains "This fork's `main` is not upstream's `main`" and a four-step "Proposing work upstream". Row added to `retro-vr-framework/docs/19-tools-index.md` (uncommitted there). The tool never names or runs the publish script: the file guard refuses that anywhere but `handoff.md` and `worker_log.md`, so `prepare` prints the arguments and the command goes into `handoff.md`.
- **Checked:** `--self-test` GREEN, 18 cases: 3 that must pass, including a clean topic whose derived commit sits directly on `upstream/main`, carries no fork-only file, keeps the executable bit, has Patrick Carey as author and committer, and equals the topic's own change; 15 that must refuse, each asserted by the reason it names, not merely by a failing exit. The first draft counted any failure as a refusal; tightened after I noticed a crash would have passed. A negative control empties FORK-ONLY and the same topic is then refused, so the filter is what excludes fork files. On real data, through the full online path (upstream URL check, fetch, `ls-remote`, the `gh` lookup, the push guard asked offline), a throwaway topic of `main` plus PR #6's commit, carrying four fork-only files, derived a patch byte-identical to `7548a80`'s (sha256 `d762fa780ea2`). The trees differ only by upstream's own README change between the two bases, because `7548a80` sits on `4a7f120` and the derive on today's `5af7c1a`. Same real data, one path left out of `--paths`: refused naming `tools/check-versions.py`; the same slug again: refused. `status` queried upstream live. The throwaway branch and ledger files were removed.
- **Found:** my first real-data check was set up wrong twice. This git's `cherry-pick` has no `-q`, so the setup failed and nothing ran; then I compared trees, which cannot match across different bases. The right invariant is patch identity plus "the tree difference is upstream's own change", and both hold. Also: the first draft wrote the staged PR body beside the tool, not into the repo being worked on; caught on reread before it ran.
- **Next / needs:** fast-forward `main` to this branch. The owner pushes the fork's `main`, which is public, so it is theirs to push.
